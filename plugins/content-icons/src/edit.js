...
export default function Edit({attributes, setAttributes}) {
	const { section_style, section_class, section_id, section_image, section_image_class, section_image_style, section_block, container_style, container_class, container_id, row_style, row_class, row_id, col_left_style, col_left_class, col_left_id,col_left_icon, col_left_title, col_left_description, col_right_style, col_right_class, col_right_id,col_right_icon, col_right_title, col_right_description } = attributes;
  ...
  />
				...
				<PanelBody title={__('Column Left')} initialOpen={false}>
					...
					<MediaUploadCheck>
  <MediaUpload
    onSelect={(media) => setAttributes({ col_left_icon: media.url })}
    type="image"
    allowedTypes={['image']}
    value={col_left_icon}
    render={({ open }) => (
      <div>
        {col_left_icon && (
          <Button
            isLink
            isDestructive
            onClick={() => setAttributes({ col_left_icon: '' })}
          >
            {__('Remove Col Image')}
          </Button>
        )}
  ...
  />
				...
				<PanelBody title={__('Column Right')} initialOpen={false}>
					...
					<MediaUploadCheck>
  <MediaUpload
    onSelect={(media) => setAttributes({ col_right_icon: media.url })}
    type="image"
    allowedTypes={['image']}
      ...
      <div>
        {col_right_icon && (
          <Button
            isLink
            isDestructive
            onClick={() => setAttributes({ col_right_icon: '' })}
          >
            {__('Remove Col Image')}
          </Button>
        )}
  ...
  <img src={col_left_icon} alt="" style={{ width: '100px', height: '100px' }} />
  <InputControl
	label="Column Left Title"
	value={col_left_title}
	onChange={(nextValue) => setAttributes({ col_left_title: nextValue })}
/>
<textarea
	id=""
	value={attributes.col_left_description}
	onChange={(event) => setAttributes({ col_left_description: event.target.value })}
	placeholder="Enter content here"
	style={{width:'100%',height:'100px'}}
/>
  </div>
  <div style={{ flex: '1', width: '50%' }}>
    <img src={col_right_icon} alt="" style={{ width: '100px', height: '100px' }} />
	<InputControl
	label="Column Right Title"
	value={col_right_title}
	onChange={(nextValue) => setAttributes({ col_right_title: nextValue })}
/>
<textarea
	id=""
	value={attributes.col_right_description}
	onChange={(event) => setAttributes({ col_right_description: event.target.value })}
	placeholder="Enter content here"
	style={{width:'100%',height:'100px'}}
/>
  </div>
